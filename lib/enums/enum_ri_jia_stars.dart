...
enum RiJiaStarEnum implements QiMenStar {
  ...
  XIAN_CHI(7, 7, "咸池", "咸"),
  TAI_YIN(8, 8, "太阴", "阴"),
  TIAN_YI(9, 9, "天乙", "天"); // 注意：双字单字名，UI 渲染时优先使用 [name]
  ...
  const RiJiaStarEnum(this.number, this.gong, this.name, this.singleCharName);

  /// 是否为吉星：太乙、青龙、太阴、天乙（用户 2026-05-04 校准）
  bool get isJi =>
      this == TAI_YI || this == QING_LONG || this == TAI_YIN || this == TIAN_YI;

  /// 是否为中平星：天符、轩辕（用户 2026-05-04 校准）
  bool get isPing => this == TIAN_FU || this == XUAN_YUAN;

  /// 是否为凶星：摄提、招摇、咸池（用户 2026-05-04 校准）
  bool get isXiong => this == SHE_TI || this == ZHAO_YAO || this == XIAN_CHI;

  static RiJiaStarEnum fromNumber(int n) =>
      values.firstWhere((e) => e.number == n,
          orElse: () =>
              throw ArgumentError('RiJiaStarEnum.fromNumber: 期望 1-9，实际 $n'));

  static RiJiaStarEnum fromName(String name) =>
      values.firstWhere((e) => e.name == name,
          orElse: () =>
              throw ArgumentError('RiJiaStarEnum.fromName: 未找到名为 $name 的日家星'));
}
