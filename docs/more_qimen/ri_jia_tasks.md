...
### 当前状态
[`ri_jia_algorithm.md`](./ri_jia_algorithm.md) 算法主体完备，§8 仍有 5 项"待补充"：

- [ ] §8.1 "九星顺飞不入中 5"的具体跳宫规则（遇 5 跳到 6？还是寄坤 2？）
- [ ] §8.2 节气过渡日（如冬至当日为非甲子日）的边界处理
- [ ] §8.3 是否需要排三奇六仪地盘？日家与时家在此差异最大
- [ ] §8.4 三吉门 / 三奇 落宫的格局判定规则
- [ ] §8.5 至少 5 个手算样例

### 子任务

#### P2-T1.1 与领域专家对齐 §8.1-§8.4 待评审项
- 以《奇门旨归》日家篇为准，逐项给出结论
...
| §8.3 三奇六仪地盘是否需要 | `EachGong.diPanGan` 字段在日家盘的语义 | 默认 nullable，UI 条件渲染；评审后再决定 |
